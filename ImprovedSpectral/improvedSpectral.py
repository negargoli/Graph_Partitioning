...
def main():
	...
	thirdmaxvalue=np.partition(flat_eigvals,2)[2]
	loc=np.where(eigvals==secondmaxvalue)[0]
	loc1=np.where(eigvals==thirdmaxvalue)[0]
	maxindex=loc[0]
	maxindex3=loc1[0]
	vect=eigvecs[:,maxindex]
        vect3=eigvecs[:,maxindex3]
	...
	for i in range(0,numberOfCells):
	 ...
	 ###################### Add improvment #################################
        for i in range (0,numberOfCells):
        	finalvect= (vect[i]/math.sqrt( (vect[i])**2 + (vect3[i])**2))*vect+ (vect3[i]/math.sqrt( (vect[i])**2 + (vect3[i])**2))*vect3
        	numberOfZeros=0
        	temp_vect2=copy.deepcopy(finalvect)
        	temp_vect2.flatten()
        	temp_vect2.sort()
        	mid_val2=temp_vect2[int(numberOfCells/2)]
        	for i in range(0,numberOfCells):
                	if(finalvect[i]<mid_val2):
                        	partition[i]=0
                	else:
                        	partition[i]=1
        	newCutSize=cutSize()
	...
	print("\n")
	CutSize=cutSize()
	print "minimumCutSize=%d" %(CutSize)
