...
void crossover(int parent1,int parent2){
	...
	/*********************** Use cutpoint to creat Offspring**************************/
      
        int cutpoint[NUM_CUTPOINT+1],i,k,count,repeat,cutpoint_temp,partition;
	count=0;
        // choose randomly cutpoints (number of cutpoints is equal to NUM_CUTPOINT)
	do{
		repeat=0;
		cutpoint_temp=rand()%number_of_cell;
		for(i=0;i<count;i++){
			if(cutpoint[i]==cutpoint_temp)	
				repeat=1;
		}
		if(!repeat){
			cutpoint[count]=cutpoint_temp;
			count++;
		}
	}while(count<NUM_CUTPOINT);

	cutpoint[NUM_CUTPOINT]=number_of_cell-1;

	partition=0;	
	count=0;
        // make offspring from putting together differnet parts of parent1 and parent2
	for(i=0;i<number_of_cell;){
		for(;i<=cutpoint[count];i++){
			if(partition==0)
   ...
   /***************************************** End of Use cutpoint to creat offspring ************************/
}
...
int stop_condition_met(){
	...
	//stop_condition=((int) (80*NUMBER_OF_POPULATION/100));
