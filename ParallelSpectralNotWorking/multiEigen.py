...
#These lines parse the input file and create the mapping array.
	...
	dijkstra_cost	= [[-1 for x in range(numberOfNets)] for y in range(numberOfCells)] 
	dijkstra_visited= [[0 for x in range(numberOfNets)] for y in range(numberOfCells)] 
...
#def graphCost(source):
#	q=deque()
#	q.appendleft(source)
#	dijkstra_cost[source][source]=0
#	dijkstra_visited[source][source]=1
#	val=0
#	while(len(q)>0):
#		data=q.pop()
#			
...
def main():
	...
	flat_eigvals.sort()
	eigvec_index= [ 0 for x in range(PARTITION)]
	for i in range(0,PARTITION):
		temp,=np.where(eigvals==flat_eigvals[i+1])#second smallest
		eigvec_index[i]=temp[0]
	vect=np.zeros([numberOfCells,PARTITION])
	for i in range(0,PARTITION):
		vect[:,i]=eigvecs[:,eigvec_index[i]]
        
	distance=np.zeros([numberOfCells]) 
	kmeans=KMeans(n_clusters=2,random_state=0).fit(vect)
       
	for i in range (0,numberOfCells):
        	for j in range (0, PARTITION):
        		distance[i]=((kmeans.cluster_centers_[0][j]-vect[i][j])**2)+distance[i] 
	half=int(numberOfCells/2)
	indx=np.argpartition(distance,-half)[-half:]     
	for i in range(0,half):
		partition[indx[i]]=1
	print(partition)
	CutSize=cutSize()
	print("Cutsize=%d", CutSize)
